...
pub async fn get_invitation(configuration: &configuration::Configuration, user_id: &str, domain_id: &str) -> Result<models::Invitation, Error<GetInvitationError>> {
    let local_var_configuration = configuration;
...
pub async fn list_invitations(configuration: &configuration::Configuration, user_id: &str, limit: Option<i32>, offset: Option<i32>, invited_by: Option<&str>, domain_id: Option<&str>, relation: Option<&str>, state: Option<&str>) -> Result<models::InvitationPage, Error<ListInvitationsError>> {
    let local_var_configuration = configuration;
    ...
    if let Some(ref local_var_str) = domain_id {
        local_var_req_builder = local_var_req_builder.query(&[("domain_id", &local_var_str.to_string())]);
    }
...
pub async fn send_invitation(configuration: &configuration::Configuration, send_invitation_req_obj: models::SendInvitationReqObj) -> Result<(), Error<SendInvitationError>> {
    let local_var_configuration = configuration;
